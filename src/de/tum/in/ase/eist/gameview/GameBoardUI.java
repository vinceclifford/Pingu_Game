...
public class GameBoardUI extends Canvas {
    ...
    public GameBoard returnCurrentGame(int index) {
        switch (index) {
            case 0 -> { return new GameBoard1(DEFAULT_SIZE); }
			case 1 -> { return new GameBoard2(DEFAULT_SIZE); }
            case 2 -> { return new GameBoard3(DEFAULT_SIZE); }
            case 3 -> { return new GameBoard4(DEFAULT_SIZE); }
            case 4 -> { return new GameBoard5(DEFAULT_SIZE); }
            case 5 -> { return new GameBoard6(DEFAULT_SIZE); }
            case 6 -> { return new GameBoard7(DEFAULT_SIZE); }
            case 7 -> { return new GameBoard8(DEFAULT_SIZE); }
        }
		return null;
    }
    ...
    private String determineMessage(){
        switch (gameAt){
            case 0 -> {return "Congratulations! You passed level 1. It was not that hard, was it?\n\n" +
            ...
            case 1 -> {return "Congratulations! You passed level 2. \n" +
                    "Let's see if you can still rescue the penguins even if KIT and LMU team up..."; }
            case 2 -> {return " Congratulations! You passed level 3.\n\n" +
                    "Harvard's number of former and current professors that won a Nobel prize totals to 50. \n" +
                    "So assuming that they would not innovate with the help of our beloved penguins is \n" +
                    "wrong. Unfortunately for us, they invented, with the help of the penguins, a sirup \n" +
                    "that makes them invisible every couple of seconds. Can you still free the penguins?";}
            case 3 -> { return "Congratulations on passing level 4!\n" +
                    "MIT is located in the backyard of Harvard. They're not even seperated by a mile.\n" +
                    ...
                    "TUM";}
            case 4 -> {return "Congratulations! You passed level 5!\n" +
                    "The west coast of the United States know of our penguins as well! Stanford combined the research \n" +
                    "of Havard and MIT and created a syrup with which one teleports and vanishes at the same time! \n" +
                    "Can you free the penguins?";}

            case 5 -> {return "Congratulations on passing level 6!\n" +
                    "Now you only need to fight off the end boss!";
            }
            case 6 -> {return "Congratulation on completing the Bumper game of ge28lop! I hope you had fun!\n" +
                    "If you still want to waste time / procrastinate you can continue playing the bonus level. In the\n" +
                    "bonus level all of the college are defending the college together. The only caveat is that you\n" +
                    "can't predict if a player in this level can vanish, teleport, move through walls etc. You'll see\n" +
                    "what I mean in a second"; }
            case 7 -> {
                return "Impressiv! Not even I completed this level!";
            }
        }
        return null;
    }
